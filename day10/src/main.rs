...
fn main() {
    ...
    //  Iterate levels 0..9 get the list of 0's as inital vec, then loop the results
    ...
    println!("Total: {total}");
    // Part 1 - 694 - correct.
    // Part 2 - 1497 - correct. Easy change to remove duplicates at each step.
}

fn paths_up_from(
    paths: &Vec<(usize, usize)>,
    steps_up: &Vec<(usize, usize)>,
) -> Vec<(usize, usize)> {
    let mut nexts: Vec<(usize, usize)> = Vec::new();

    for (x, y) in paths {
        if steps_up.contains(&(x + 1, *y)) {
            nexts.push((x + 1, y.clone()));
        }
        if *x > 0 && steps_up.contains(&(x - 1, *y)) {
            nexts.push((x - 1, y.clone()));
        }
        if steps_up.contains(&(*x, y + 1)) {
            nexts.push((x.clone(), y + 1));
        }
        if *y > 0 && steps_up.contains(&(*x, y - 1)) {
            nexts.push((x.clone(), *y - 1));
        }
    }
    nexts
